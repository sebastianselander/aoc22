...
inp = unsafePerformIO $ readFile "src/Day11/input.txt"
ex = unsafePerformIO $ readFile "src/Day11/ex.txt"


modder = 17 * 13 * 19 * 7 * 11 * 3 * 2 * 5
...
instance Show Monkey where
  show (Monkey s o te true false ins) = "Monkey " <> show s <> " " <> show true <> " " <> show false <> " " <> show ins
...
monkeyPlay f monkey
  ...
  | otherwise = let playItem = head $ starting monkey
                 ...
                 in let playItem' = f (op playItem)
                    in if te playItem'
                         then (Just (true,playItem'), Monkey (tail $ starting monkey) op (test monkey) true false (ins+1))
                         else (Just (false, playItem'), Monkey (tail $ starting monkey) op (test monkey) true false (ins+1))

playRound :: (Integer -> Integer) -> (Integer -> Integer) -> Integer -> M.Map Integer Monkey -> M.Map Integer Monkey
playRound modder f n m = case M.lookup n m of
    ...
    (Just mnky) -> let (perhaps,monkey) = monkeyPlay f mnky
                   in case perhaps of
                       ...
                       Just (to,item) -> playRound modder f n (adjustMnky modder (to,item) (M.insert n monkey m))

adjustMnky :: (Integer -> Integer) -> (Integer, Integer) -> M.Map Integer Monkey -> M.Map Integer Monkey
adjustMnky modder (to, item) m = M.adjust (addItem modder item) to m

solve1 :: String -> String
solve1 = show . product . take 2 . sortBy (flip compare) . map (inspected . snd) . M.toList . last . take 21 . iterate (playRound id (flip div 3) 0) . toMonkeys

solve2 :: String -> String
solve2 str = show . product . take 2 . sortBy (flip compare) . map (inspected . snd) . M.toList . last . take 10001 . iterate (playRound modder id 0) $ toMonkeys str
    where
